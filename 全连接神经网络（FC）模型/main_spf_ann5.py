...
import h5py
import os
import geshi_spf_ann2 as geshispfann2
import geshi_spf_ann as geshispf2
import MergeMeanShift
import re
    ...
    with h5py.File(h5_path, 'w') as f:
        with tqdm(total=num_samples, desc="生成训练数据", unit="sample") as pbar:
            for i in range(num_samples):        #生成xxx个样本
                # 只生成1个干扰源
                receivers, spoofers = geshispf2.generation_data(
                    x_min=0, x_max=1000, y_min=0, y_max=1000,
                    num_receivers=2000, num_spoofers=1
                )
...
class NeuralLocalizer:
    ...
    def predict(self, receivers):
        ...
        # 2. 方差
        var_x = np.var(interfered[:,0])
        var_y = np.var(interfered[:,1])
        
        # 3. 长宽比
        min_x, max_x = np.min(interfered[:,0]), np.max(interfered[:,0])
        min_y, max_y = np.min(interfered[:,1]), np.max(interfered[:,1])
        """ # 如果没有受干扰的接收机，返回空数组
        if len(interfered) == 0:
            return np.empty((0, 2))
        ...
        # 计算特征
        # 1. 中心坐标
        centroid_x = np.mean(interfered['x'])
        centroid_y = np.mean(interfered['y'])
        ...
        # 计算左下侧接收机点数目
        left_bottom_count = np.sum((interfered[:,0] <= left_boundary) & (interfered[:,0] >= min_x)
                            & (interfered[:,1] >= min_y)& (interfered[:,1] <= bottom_boundary))
        # 计算左上侧接收机点数目
        left_top_count = np.sum((interfered[:,0] <= left_boundary) & (interfered[:,0] >= min_x)
                                & (interfered[:,1] >= top_boundary) & (interfered[:,1] <= max_y))
        
        # 计算右下侧接收机点数目
        right_bottom_count = np.sum((interfered[:,0] <= max_x) & (interfered[:,0] >= right_boundary)
                             & (interfered[:,1] >= min_y)& (interfered[:,1] <= bottom_boundary))
        # 计算右上侧接收机点数目
        right_top_count = np.sum((interfered[:,0] <= max_x) & (interfered[:,0] >= right_boundary)
                                  & (interfered[:,1] >= top_boundary) & (interfered[:,1] <= max_y))
        interfered_num = len(interfered)
        lbc_ratio = left_bottom_count / interfered_num if interfered_num > 0 else 1.0 
        lrc_ratio = left_top_count / interfered_num if interfered_num > 0 else 1.0
        rbc_ratio = right_bottom_count / interfered_num if interfered_num > 0 else 1.0
        rtc_ratio = right_top_count / interfered_num if interfered_num > 0 else 1.0    
...
def data_read(receivers_with_cnr):
    ...
    spoof_records = []
   # 遍历每个接收机
    for i in range(len(receivers_with_cnr)):
        receiver = receivers_with_cnr[i]
        base_info = {
            'y': float(receiver['y']),
            'x': float(receiver['x']),
            'spoofNum': receiver['spoofer_count']
        }
        # 解析欺骗干扰信息
        if receiver['spoofer_count'] > 0:
            # 处理字符串类型的欺骗索引（兼容逗号分隔格式）
            spoofer_indices = receiver['spoofer_types']
            if isinstance(spoofer_indices, str):
                # 分割并去除每个元素的空格和单引号
                spoofer_indices = [item.strip("' ").strip() for item in spoofer_indices.split(',')]
            
            # 遍历处理后的欺骗索引
            for spoofer_idx in spoofer_indices:
                record = base_info.copy()
                record['spoofer_types'] = spoofer_idx
                spoof_records.append(record)   

    result_spoof = pd.DataFrame(spoof_records)if spoof_records else pd.DataFrame(columns=['y', 'x', 'spoofNum', 'spoofer_indices'])
    if not result_spoof.empty:
        #result_spoof['spoofer_indices']=result_spoof['spoofer_indices'].str.strip("'")
        result_spoof['spoofer_types']= result_spoof['spoofer_types'].astype(int)
    return  result_spoof

def spoof_clustering_main(data_test_interfered):
    data_test_interfered['x'] = pd.to_numeric(data_test_interfered['x'], errors='coerce')
    data_test_interfered['y'] = pd.to_numeric(data_test_interfered['y'], errors='coerce')
    
    X = np.array(data_test_interfered[['x','y']])
    weights = [1 / len(X)] * len(X)
    # 定义候选带宽范围（根据数据特性调整）
    '''
    def generate_bandwidths(X, base_values=[100,200,300,500, 700, 900], n=10):
        # 计算数据特征
        ...
        # 动态范围
        min_bw = max(100, 2*median_dist)
        max_bw = min(900, 0.2*(X.max()-X.min()))
        
        # 生成候选
        auto_values = np.linspace(min_bw, max_bw, n-len(base_values)).astype(int)
        return sorted(set(base_values + auto_values.tolist()))

    bandwidth_mixed = generate_bandwidths(X)
    #print("智能混合生成:", bandwidth_mixed)
    bandwidth_candidates = bandwidth_mixed
    ...
    for bw in bandwidth_candidates:
        try:
            ...
            ems = MergeMeanShift.SelfMergeMeanShift(bandwidth=bw, weights=weights)
            #ems  = MeanShift(bandwidth=bw, weights=weights)
            #ems = MergeMeanShift.SelfMergeMeanShift(bandwidth=bw, cnr_values=interfered_cnr, temperature=0.3)
            ems.fit(X)
            
            # 计算轮廓系数（需要至少2个聚类）
            n_clusters = len(ems.centroids)
            if n_clusters > 1:
                ch_score = calinski_harabasz_score(X, ems.labels_)
                status = f"有效聚类，轮廓系数 = {ch_score:.3f}"
            else:
                ch_score = -1
                status = "无效聚类（单聚类）"
            # 记录最佳结果
            if ch_score > best_ch_score:
                ...
                best_bandwidth = bw
                #best_labels = ems.labels_
                #best_centroids = ems.centroids
                best_ems = ems
            #logger.info(f"Bandwidth: {bw:.4f} | 聚类数 {n_clusters:2d} | Silhouette Score: {sil_score:.3f}")
            #print(f"带宽 {bw:4d} | 聚类数 {n_clusters:2d} | {status}")
    
        except Exception as e:
            logger.warning(f"Bandwidth {bw} failed: {str(e)}")
            continue
    # 使用最优参数最终聚类
    #print(f"\n最优带宽: {best_bandwidth}, 最高轮廓系数: {best_ch_score:.3f}")
    ems = best_ems
    #print(f"Optimal bandwidth: {best_bandwidth:.3f}")
    print(f"Identified {len(ems.centroids)} clusters with Silhouette Score: {best_ch_score:.3f}")
    #print("Cluster centers:\n", ems.centroids)
    '''
    best_bandwidth=150
    # 执行聚类（传入CNR值）
    bandwidth = best_bandwidth
    ems = MergeMeanShift.SelfMergeMeanShift(bandwidth=bandwidth, weights=weights)
    ems.fit(X)      

    # 保存聚类标签到数据
    data_test_interfered['label'] = ems.labels_
    cluster_centers = ems.centroids

    return data_test_interfered,cluster_centers

def process_sg_group(args):
    sg, group = args
    # 对当前sg组的接收机数据进行聚类
    data_test_interfered,cluster_centers = spoof_clustering_main(group)

    return  data_test_interfered,cluster_centers
# 清理函数
def clean_sg(sg):
    cleaned = re.sub(r'[^\d]', '', str(sg))  # 移除非数字字符
    return cleaned

def spoof_interfer_estimation(result):
    if result['spoofNum'].sum() != 0:
        
        groups = result.groupby(['spoofer_types'])
        
        # 初始化空列表收集所有结果
        all_data_frames = []  # 收集每个任务的DataFrame
        all_cluster_centers = []  # 收集聚类中心
        #spfCenter = []
        #cluster_centers = []    
        # 并行处理每个特征组
        with ProcessPoolExecutor() as executor:
            args_list = [(clean_sg(sg), group) for sg, group in groups]
            futures = [executor.submit(process_sg_group, args) for args in args_list]
            
            for future in futures:
                data_interfered,cluster_centers = future.result()
                # 确保每个任务返回的是DataFrame
                if isinstance(data_interfered, pd.DataFrame):
                    all_data_frames.append(data_interfered)
                else:
                    # 如果不是DataFrame，尝试转换
                    all_data_frames.append(pd.DataFrame(data_interfered))
                
                all_cluster_centers.extend(cluster_centers)
        
        # 合并所有DataFrame
        if all_data_frames:
            data_test_interfered = pd.concat(all_data_frames, ignore_index=True)
        else:
            data_test_interfered = pd.DataFrame()
    else:
        data_test_interfered = []
        all_cluster_centers = []

    return  data_test_interfered, all_cluster_centers
...
def generate_data(path, num_samples):
    """生成测试数据"""
...
if __name__ == "__main__":
    ...
    with tqdm(range(num_simulations), desc="运行仿真", unit="sim") as sim_pbar:
        for sim in sim_pbar:
            ...
            cluster_errors = calculate_paired_errors(spoofers, cluster_centers)
            #print(errors)
            #print(cluster_errors)
            all_errors.extend(errors['distances'])
            all_cluster_errors.extend(cluster_errors['distances'])
            '''
            ...
            cluster_squared_errors = [e ** 2 for e in cluster_errors['distances']]
            error2.extend(squared_errors)  # 添加到 error2 列表
            cluster_error2.extend(cluster_squared_errors)
            #print(all_errors)
            ...
            # 绘制预测干扰源
            if len(predicted_spoofers) > 0:
                plt.scatter(predicted_spoofers[:, 0], predicted_spoofers[:, 1], marker='X',
                            s=200, c='lime', edgecolors='black',
                            linewidths=1, label='预测干扰源')
                
                # 添加误差线
                for i in range(len(spoofers)):
                    plt.plot([spoofers[i, 0], predicted_spoofers[i, 0]], 
                             [spoofers[i, 1], predicted_spoofers[i, 1]],
                             'r--', alpha=0.7)
    ...
    cluster_max_error = np.max(all_cluster_errors)
    mean_cluster_error = np.mean(all_cluster_errors)if all_cluster_errors else 0
    cluster_errors2 = np.array(cluster_error2)
    mean_cluster_error2= np.mean(cluster_errors2)
    cluster_RMSE_error = np.sqrt(mean_cluster_error2)if mean_cluster_error2 else 0
    #print(f" 聚类平均误差为 {mean_cluster_error:.2f}m, 聚类RMSE为 {cluster_RMSE_error:.2f}m")
    #print(f" 聚类最大误差为 {cluster_max_error:.2f}m")
    ...
    plt.hist(all_errors, bins=bins, alpha=0.6, color='red',
        label=f'({mean_error:.1f}, {RMSE_error:.1f})', density=True)
    ...
    plt.figure(figsize=(6, 6))
    bins = np.linspace(0, np.max(all_cluster_errors)+10, 30)
    max_error = np.max(all_cluster_errors)
    bins = np.arange(0, max_error + 10, 10)  # 区间宽度固定10米
    plt.hist(all_cluster_errors, bins=bins, alpha=0.6, color='blue', 
            label=f'({mean_cluster_error:.1f}, {cluster_RMSE_error:.1f})', density=True)
    #        label=f'($\sigma^2$={cluster_var:.1f} m$^2$)', density=False)
    #plt.hist(cluster_errors, bins=bins, alpha=0.6, color='red',
    #        label=f'聚类中心 (μ={cluster_mean:.1f}, σ²={cluster_var:.1f})', density=True)
    plt.xticks(bins)  # 显示所有刻度
    #plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{x:.0f}'))

    plt.xlabel('error (m)')
    plt.ylabel('probability density ')
    #plt.title('定位误差分布直方图')Histogram of positioning error distribution
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.show()
