...
class InterferenceDataset(Dataset):
    ...
    def __init__(self, h5_path):
        ...
        self.grid_size = []  # 网格大小
        self.grid_transforms = []  # 存储网格变换参数 (X_center, Y_center, L)
        # 数据加载与处理
        with h5py.File(h5_path, 'r') as f:
            ...
            for key in tqdm(f.keys(), desc="加载数据集"):
                ...
                Ly=Y_range/100
                L=np.maximum(Lx,Ly)
                #self.grid_size.append(L)
                #print(L)
                # 保存网格变换参数 (用于全局坐标系下的相对位置编码)
                ...
                for i, receiver in enumerate(affected_receivers):
                    ...
                    # 计算网格索引 (0-99)
                    i_idx = min(int((y-Y_center+50*L)/L), 99)
                    j_idx = min(int((x-X_center+50*L)/L), 99)
                    # 累加干扰功率
                    # 确保jam_pw[i]是标量值
    ...
    def __getitem__(self, idx):
        ...
        # 归一化网格变换参数
        grid_params = torch.tensor([X_center/1000.0, Y_center/1000.0, L/1000.0], dtype=torch.float32)
...
class InterferenceModel(nn.Module):
    def __init__(self, cnn_output_channels=3, cnn_output_size=10):
        ...
        self.fc_A_enhanced = nn.Sequential(
            nn.Linear(self.flat_features + 3, 128),  # 额外添加3个网格参数特征
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.BatchNorm1d(64),
        ...
        self.confidence_fc_enhanced = nn.Sequential(
            nn.Linear(self.flat_features + 3, 64),  # 额外添加3个网格参数特征
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.BatchNorm1d(32),
...
def train_model():
    ...
    for epoch in range(30):
        ...
        #     torch.save(model.state_dict(), "best_cnn2model_stage1.pth")
    ...
    for epoch in range(30):
        ...
        # if avg_val_loss < best_val_loss:
        #     best_val_loss = avg_val_loss
        #     torch.save(model.state_dict(), "best_cnn2model_stage2.pth")
    ...
    for epoch in range(20):
        ...
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(model.state_dict(), "best_cnn2model435_stage3.pth")
...
class NeuralLocalizer:
    ...
    def predict(self, receivers):
        ...
        # 准备网格参数（用于全局坐标系下的相对位置编码）
        grid_params = torch.tensor([X_center/1000.0, Y_center/1000.0, L/1000.0], dtype=torch.float32).unsqueeze(0).to(device)
...
if __name__ == "__main__":
    ...
    # 绘制CNN预测误差直方图
    plt.hist(all_errors_cnn, bins=bins, alpha=0.5, color='orange',
                label=f'CNN预测 (均值: {mean_error_cnn:.1f}m, RMSE: {RMSE_error_cnn:.1f}m)', 
                density=True)
    
    # 绘制WCL位置误差直方图
    plt.hist(all_errors_wcl, bins=bins, alpha=0.5, color='purple',
